...
@pytest.mark.parametrize("name, mkgen, Lmax", [
    ("Tausworthe k=31",
     lambda: Generateur.create("Tausworthe", 32, poly=[0, 13, 31], s=18, quicktaus=True),
     32),
    ("MELG607-64",
     lambda: Generateur.create("MELG", 64, w=64, N=10, M=5, r=33,
                                sigma1=13, sigma2=35, a=0x81F1FD68012348BC),
     64),
])
def test_simd_collapses_to_dual_lattice_for_full_period_non_simd(name, mkgen, Lmax):
    gen = mkgen()
    C = Combinaison.CreateFromFiles([[gen]], Lmax=Lmax, temperings=[[]])
    C.reset()
...
def test_simd_sfmt_matches_mttoolbox(mexp):
    ...
    gen = Generateur.create("SFMT", 32, mexp=mexp,

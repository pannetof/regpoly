...
class EquidistributionResults(AbstractTestResults):
    ...
    def display_table(self, C: "Combination", by: str = 'l') -> tuple[str, int]:
        """
        ...
        if by == 'l':
            ...
            dual_label = "DIM    "
            def dual(i: int) -> int:
                return min(C.k_g, C.k_g // i) - table[i]
        elif by == 't':
            self._set_phi12(C)
    ...
    def _set_phi12(self, C: "Combination") -> None:
        ...
        for l in range(1, r2 + 1):
            phi[min(C.k_g, C.k_g // l)] = True
        self._phi12 = phi

    def _conv_ecarts(self, C: "Combination") -> list[int]:
        """ConvEcarts: convert dimension gaps Delta_l to resolution gaps Lambda_t."""
        lam  = [-1] * (C.k_g + 1)
        for t in range(1, C.k_g + 1):
            l = min(C.k_g // t, self.L)
            for i in range(1, l + 1):
                t_i = min(C.k_g // i, C.k_g) - self.ecart[i]
                if t <= t_i:
                    lam[t] = l - i
        return lam

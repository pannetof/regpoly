...
config), builds the Combination, and runs the search loop — mirroring
...
def _search_one_combo(conn, run_id: int, comb: Combination, test,
...
def _save_tested_result(conn, run_id: int, comb: Combination,
...
def _find_primitive_id(conn, gen: Generator) -> int | None:
...
def _build_gen_and_trans(components: list[dict], Lmax: int):
    """Build per-component lists of Generator and Transformation instances.
    ...
    :func:`regpoly.core.combination_build.build_tempering_chain` so it is
    shared with the library runner.
    """
    from regpoly.core.combination_build import build_tempering_chain

    gen_pools: list[list[Generator]] = []
    pool_by_index: dict[int, list[Generator]] = {}
        ...
        if c["shared_with_component"] is not None \
            ...
            for g in c["generators"]:
                pool.append(
                    ...
                    # paper / test assumes.
                    Generator.create(g["family"], Lmax, **g["all_params"])
                )

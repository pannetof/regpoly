...
Combination is a Python iterator over all valid combinations:
...
Generator object does not appear twice — if it did, the XOR of the
...
comb[j] returns the active Generator of component j.
"""
...
class Combination:
    ...
    def CreateFromFiles(
        ...
        """
        Build a fully-populated Combination from pre-built data.

        Parameters
        ----------
        gen_lists  : list[list[Generator]] — pre-built generator pool per component;
        ...
        Returns
        -------
        comb : Combination — fully initialised and populated
        """
    ...
    def __iter__(self) -> "Combination":
        """Set up a fresh iterator; the first next() call yields combo 1."""
        self._combo_iter = self._make_combo_iter()
        return self

    def __next__(self) -> "Combination":

...
// state across recursion steps in a way SFMT's stateless update does not).
//
...
// simd_reset_word_index following the SFMT pattern but with the dSFMT
// recurrence and the lung carry.
class dSFMTGen : public Generateur {
public:
    ...
    // the stream by lane within a state-cycle (analogous to SFMT's
    ...
    // Set state without triggering generate_all (matches SFMT semantics

...
XorShift128::XorShift128(int a, int b, int c, int L, int pattern)
    : Generateur(128, L), a_(a), b_(b), c_(c), pattern_(pattern) {}

std::string XorShift128::name() const { return "XorShift128"; }

std::string XorShift128::display_str() const {
    char buf[64];
...
void XorShift128::load_state(uint32_t& x, uint32_t& y,
...
void XorShift128::store_state(uint32_t x, uint32_t y,
...
void XorShift128::init(const BitVect& init_bv) {
    state_ = BitVect(k_);
    state_.copy_part_from(init_bv, k_);
}

void XorShift128::next() {
    uint32_t x, y, z, w;
    load_state(x, y, z, w);
    uint32_t t;
    if (pattern_ == 0) {
...
std::unique_ptr<Generateur> XorShift128::copy() const {
    auto p = std::make_unique<XorShift128>(a_, b_, c_, L_, pattern_);
    p->state_ = state_.copy();
    return p;
}

std::unique_ptr<Generateur> XorShift128::from_params(
    ...
    return std::make_unique<XorShift128>(a, b, c, std::min(L, 32), pattern);
}

std::vector<ParamSpec> XorShift128::param_specs() {
    return {

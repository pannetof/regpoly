...
MTGP::MTGP(int mexp, int pos, int sh1, int sh2, uint32_t mask,
...
{
    if (tbl_.size() != 16)
        throw std::invalid_argument("MTGP: tbl must have 16 entries");
    if (tmp_tbl_.size() != 16)
        throw std::invalid_argument("MTGP: tmp_tbl must have 16 entries");
    state_ = BitVect(32 * N_);
}

std::string MTGP::name() const { return "MTGP"; }

std::string MTGP::display_str() const {
    char buf[160];
...
void MTGP::init(const BitVect& init_bv) {
    state_ = BitVect(32 * N_);
...
std::unique_ptr<Generateur> MTGP::copy() const {
    auto p = std::make_unique<MTGP>(mexp_, pos_, sh1_, sh2_,
...
std::unique_ptr<Generateur> MTGP::from_params(
    const Params& params, int L)
{
    ...
    if (tbl.size() != 16)
        throw std::invalid_argument(
            "MTGP: 'tbl' must be a 16-element uint_vec");
    if (tmp_tbl.size() != 16)
        throw std::invalid_argument(
            "MTGP: 'tmp_tbl' must be a 16-element uint_vec");
    std::vector<uint32_t> t32(16), tt32(16);
    for (int i = 0; i < 16; i++) {
        t32[i]  = (uint32_t)tbl[i];
        tt32[i] = (uint32_t)tmp_tbl[i];
    }
    return std::make_unique<MTGP>(mexp, pos, sh1, sh2, mask,
                                  t32, tt32, L);
}

std::vector<ParamSpec> MTGP::param_specs() {
    return {

...
#include "poly_lattice.h"    // PolyLatBase — dual-lattice reduction
#include <algorithm>
#include <climits>
#include <memory>
#include <random>
...
// single Generateur of dimension kg = Σ k_j.
// ─────────────────────────────────────────────────────────────────────
class CombinedView : public Generateur {
public:
    CombinedView(const std::vector<Generateur*>& gens,
                 const std::vector<std::vector<Transformation*>>& trans,
                 int kg, int L)
        : Generateur(kg, L), trans_(trans)
    {
    ...
    std::unique_ptr<Generateur> copy() const override {
        std::vector<Generateur*> raw_gens;
...
private:
    std::vector<std::unique_ptr<Generateur>> gens_;
    ...
    const std::vector<int>& delta, int mse)
{
    ...
    // analytical char_poly() overrides (MT, MELG, Tausworthe, …) this
    // gives the EXACT χ_f; select_phi then identifies the largest
    // Mersenne primitive factor and the fast path triggers.
    //
    // For generators with no analytical char_poly() (SFMT defaults to
    ...
    // (e.g. SFMT-607's φ_607 lives in lanes other than u[0] bit 31).
    //
    ...
    // LCM.  This catches SFMT-style cases where the analytical char
    ...
    for (size_t j = 0; j < gens.size(); j++) {
        std::vector<Generateur*> single = {gens[j]};
    ...
    auto read_combined_now = [&](std::vector<std::unique_ptr<Generateur>>& gen_copies) {
        BitVect combined(L);
        for (size_t j = 0; j < gens.size(); j++) {
    ...
    for (int sm = 0; sm < Phi; sm++) {
        ...
        PolyLatBase base(RES_sm, Deg_sm);

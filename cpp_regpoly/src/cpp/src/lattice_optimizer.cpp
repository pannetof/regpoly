...
static BitVect get_transformed_output_local(
    Generateur* gen, const std::vector<Transformation*>& chain)
{
    if (chain.empty())
...
// ══════════════════════════════════════════════════════════════════════════
// LatticeOptCache implementation
// ══════════════════════════════════════════════════════════════════════════

LatticeOptCache::LatticeOptCache(
...
std::vector<int> LatticeOptCache::compute_all() {
    std::vector<int> ecart(L_ + 1, -1);

    auto base = std::make_unique<PolyLatBase>(RES_, Deg_);
    ...
    for (int l = 1; l <= RES_; l++) {
        ...
        stack_[l] = std::make_unique<PolyLatBase>(*base);
...
std::vector<int> LatticeOptCache::rebuild() {
...
void LatticeOptCache::reset_step() {
    previous_v_ = 0;
    current_base_.reset();
    opt_stack_.clear();
...
int LatticeOptCache::step(int v) {
    ...
    if (v == 1) {
        ...
        current_base_ = std::make_unique<PolyLatBase>(RES_, Deg_);
        ...
        opt_stack_[v - 2] = std::make_unique<PolyLatBase>(*current_base_);
...
void LatticeOptCache::find_single_poly(int idx, BitVect& out_poly) {
    // Collect output bit 'idx' from the generator for K steps
    int K = Deg_;
    int J = (int)gens_.size();

    std::vector<std::unique_ptr<Generateur>> gen_copies;
    for (int j = 0; j < J; j++)
...
void LatticeOptCache::normalize_single(BitVect& poly) {
...
void LatticeOptCache::refresh_inv_g0() {
...
int LatticeOptCache::compute_gap(int v) {
    if (v < 1 || v > RES_) return 0;

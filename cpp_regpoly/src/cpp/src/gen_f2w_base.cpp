...
std::string GenF2wBase::display_str() const {
...
uint64_t GenF2wBase::multiply(uint64_t a, uint64_t b) const {
    if (normal_basis_)
...
uint64_t GenF2wBase::V(int idx) const {
    return state_.get_word(idx, w_);
}

void GenF2wBase::SetV(int idx, uint64_t val) {

...
def structural_filename(gen: Generateur) -> str:
...
def save_tested_generator(
    ...
    comb       : the Combinaison that was tested
...
def load_tested_generator(filepath: str) -> tuple[Combinaison, dict]:
    """
    Load a tested generator YAML file.

    Returns (Combinaison, results_dict).
    The Combinaison is ready to use (generator created, tempering applied).
    """
...
def _single_component_data(comb: Combinaison, j: int) -> dict:
    gen = comb[j]
...
def _multi_component_data(comb: Combinaison) -> dict:
    components = []
...
def _load_multi(data: dict) -> Combinaison:
    ...
    comb = Combinaison(J=J, Lmax=Lmax)

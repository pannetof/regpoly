...
class BitVect:
    ...
    def __init__(self, n: int, val: int = 0):
        ...
        self.n: int = n
        self._val: int = val & ((1 << n) - 1)

    @classmethod
    def zeros(cls, n: int) -> "BitVect":
        """AllocBV / PutBVToZero: create a zero-filled bit vector of n bits."""
        return cls(n, 0)

    @classmethod
    def canonic(cls, n: int, l: int) -> "BitVect":
        """
    ...
    @classmethod
    def all_ones(cls, n: int) -> "BitVect":
        """AllOnes: create an n-bit vector with every bit set to 1."""
        return cls(n, (1 << n) - 1)

    @classmethod
    def mask(cls, n: int, l: int) -> "BitVect":
        """
    ...
    @classmethod
    def invmask(cls, n: int, l: int) -> "BitVect":
        """
    ...
    @classmethod
    def random(cls, n: int) -> "BitVect":
        """RandVect: create a random n-bit vector."""
        return cls(n, _random.getrandbits(n))
    ...
    def copy(self) -> "BitVect":
        ...
        return BitVect(self.n, self._val)
    ...
    def __xor__(self, other: "BitVect") -> "BitVect":
        ...
        return BitVect(self.n, self._val ^ other._val)
    ...
    def __invert__(self) -> "BitVect":
        ...
        return BitVect(self.n, self._val ^ ((1 << self.n) - 1))
    ...
    def xor3(self, b: "BitVect", c: "BitVect") -> "BitVect":
        ...
        return BitVect(self.n, self._val ^ b._val ^ c._val)
    ...
    def and_mask(self, t: int) -> "BitVect":
        ...
        """
        if t <= 0:
            return BitVect(self.n, 0)
        if t >= self.n:
            return self.copy()
        return BitVect(self.n, self._val & (((1 << t) - 1) << (self.n - t)))
    ...
    def and_invmask(self, t: int) -> "BitVect":
        ...
        if t >= self.n:
            return BitVect(self.n, 0)
        return BitVect(self.n, self._val & ((1 << (self.n - t)) - 1))
    ...
    def __lshift__(self, k: int) -> "BitVect":
        ...
        if k >= self.n:
            return BitVect(self.n, 0)
        return BitVect(self.n, (self._val << k) & ((1 << self.n) - 1))
    ...
    def __rshift__(self, k: int) -> "BitVect":
        ...
        if k >= self.n:
            return BitVect(self.n, 0)
        return BitVect(self.n, self._val >> k)
    ...
    def lrot_shift(self, k: int, w: int) -> "BitVect":
        ...
        top = self._val >> (self.n - w)
        bottom = self._val & ((1 << (self.n - w)) - 1)
        rotated = ((top << k) | (top >> (w - k))) & ((1 << w) - 1)
        return BitVect(self.n, (rotated << (self.n - w)) | bottom)

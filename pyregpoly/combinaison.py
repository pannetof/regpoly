...
class Combinaison:
    ...
    def _get_dispatch(cls):
        if not cls._GEN_DISPATCH:
            from polylcg import PolyLCG
            from tausworthe import Tausworthe
            from tgfsr import TGFSR
            cls._GEN_DISPATCH = {
                "polylcg": PolyLCG,
                "taus"   : Tausworthe,
                "taus2"  : Tausworthe,
                "tgfsr"  : TGFSR,
                # "genf2w" : GenF2w,        # not yet implemented
                # "MT"     : MT,            # not yet implemented
            }
    ...
    def display(self) -> None:
        """Print a summary of the current combined generator."""
        print("=" * 66)
        print(f"Number of points   : 2^({self.k_g:3d})")
        if self.smax != sys.maxsize:
            print(f"Maximum dimension : {self.smax:3d}")
        print()

        poly_bv = self[0].char_poly()
        print(f"hammingweight = {bin(poly_bv._val).count('1')}")
        ...
        for j, comp in enumerate(self.components):
            ...
            active.display()
            comp.display()
            print("=" * 66 if j == self.J - 1 else "-" * 66)

...
from tuplets_results import TupletsResults, _MAX_TYPE, _SUM_TYPE
from equidistribution_test import EquidistributionTest
...
class TupletsTest(AbstractTest):
    ...
    def run(self, C: Combinaison, *args, **kwargs) -> TupletsResults:
        ...
        Returns a TupletsResults with is_ok() == True when tupletsverif is False
        (test skipped) or when the generator passes the criterion.
        """
        ...
        # For L <= 32, use word-level matrix to replicate C's 64-bit ulong
        # overflow behaviour in Gaussian elimination.
        use_words = (L <= 32)
        if use_words:
            working = self._prepare_mat_words(C, indice_max)
            _equid  = EquidistributionTest._resolution_equid_words
        else:
            mat     = self._prepare_mat(C, indice_max)
            working = np.asarray(mat._mat, dtype=np.int64).copy()
            _equid  = EquidistributionTest._resolution_equid
        ...
        for dim in range(1, maxindsucc + 1):
            ...
            l_t      = _equid(working, kg, dim, L, indices)
        ...
        if first_ok and tupd > 1:
            for dim_idx in range(2, tupd + 1):
                ...
                for rest in _itertools_combinations(range(1, tuph[dim_idx]), dim_idx - 1):
                    indices = [0] + list(rest)
                    l_t     = _equid(working, kg, dim_idx, L, indices)
                    gap_val = bound - l_t

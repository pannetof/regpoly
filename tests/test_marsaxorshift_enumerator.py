...
the corresponding ``ParamBag``.  These tests verify the cardinalities,
...
def _make_enum(typ: int, w: int, r: int, **extra) -> "cpp.ParameterSpaceEnumerator":
    ...
    params.update(extra)
    e = cpp.make_parameter_space_enumerator("MarsaXorshiftGen", params, 32)
...
def test_type1_rejects_r_neq_1():
    with pytest.raises(ValueError, match=r"needs_r_eq_1"):
        cpp.make_parameter_space_enumerator(
...
def test_table_iii_gen1_appears_in_type2_enumeration():
    ...
    checking uniqueness of every emitted ``ParamBag``."""
    w, r = 32, 2

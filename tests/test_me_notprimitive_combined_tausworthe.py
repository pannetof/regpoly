...
def _build_combination(components: list[dict], Lmax: int):
    ...
    active = [pool[0] for pool in gen_lists]
    comb = make_combined(*active, trans=temperings, Lmax=Lmax)
    gens = [g._cpp_gen for g in active]
    trans = [
        [t._cpp_trans for t in chain if hasattr(t, "_cpp_trans")]
        for chain in temperings
    ]
...
def test_lfsr258_no_longer_raises():
    ...
    assert comb.k() == 258, comb.k()

    res = cpp.test_me_notprimitive(
        comb,
        comb.k(), comb.L(), comb.L(),
        [0] * (comb.L() + 1), 0,
    )

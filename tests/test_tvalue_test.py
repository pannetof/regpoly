...
def test_generator_create_niederreiter():
    g = Generator.create("NiederreiterF2Net", L=2, s_max=2)
    assert g._type_name == "NiederreiterF2Net"
    assert g.L == 2
...
def _make_combination(family: str, m: int, s_max: int):
    gen = Generator.create(family, L=m, s_max=s_max)
    return make_combined(gen, Lmax=m)
...
def test_run_on_niederreiter_m2_s2_gives_zero():
    C = _make_combination("NiederreiterF2Net", m=2, s_max=2)
    res = TValueTest(s_max=2, max_t_sum=sys.maxsize).run(C)
    assert res.verified
    assert res.tvals[2] == 0
...
def test_equidistribution_runs_unchanged_on_niederreiter():
    ...
    C = _make_combination("NiederreiterF2Net", m=4, s_max=4)
    eqt = EquidistributionTest(L=4, delta=[sys.maxsize] * 5, mse=sys.maxsize)
    res = eqt.run(C)
    assert res.verified
    assert res.se >= 0

...
from regpoly.library import bundled_library_dir

REPO_ROOT = Path(__file__).resolve().parents[1]
DOCS_LIB = bundled_library_dir()
...
# NOTE: this whole cross-check is slow-lane only. It was silently dormant
# from the monorepo's creation until the v2.x two-repo split (its DOCS_LIB
# pointed at a path that never existed, so it collected zero cases). The
# split relocated the catalog into package data and fixed the path, which
# activated it — and revealed that some cases (e.g. mt-mt19937) take ~150s
# and currently disagree with the frozen MTToolBox reference. Pending a
# revalidation of the reference data / equidistribution method, the cross
# check runs in the slow lane only so it never blocks the default lane.
# The per-case `slow` flag (mexp > slow_threshold_mexp) is kept for when
# the references are re-baselined.
@pytest.mark.slow
...
def test_regpoly_matches_mttoolbox_d5(case: dict) -> None:
    ...
    C = make_combined(gen, trans=[temper_chain], Lmax=L)

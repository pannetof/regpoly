...
"""Integration tests for ``regpoly.core.parametric.build_parameter_space_enumerator``
...
``regpoly.core.parametric.build_parameter_space_enumerator`` strips the pybind11
...
from regpoly.core.parametric import Enumerator, NotEnumerable, build_parameter_space_enumerator


def test_returns_enumerator_for_complete_type1_inputs():
    e = build_parameter_space_enumerator("MarsaXorshiftGen", L=32,
...
def test_NotEnumerable_with_needs_reason(resolved, expected_reason):
    with pytest.raises(NotEnumerable) as excinfo:
        build_parameter_space_enumerator("MarsaXorshiftGen", L=32, resolved=resolved)
    assert excinfo.value.reason == expected_reason


def test_unsupported_family_returns_None():
    # WELLGen has no enumerator registered.
    e = build_parameter_space_enumerator("WELLGen", L=32, resolved={})
    assert e is None

...
TemperingTryResult round-trips correctly, and the iteration counts
line up.
"""
...
def test_run_tempering_search_early_stop_on_zero() -> None:
    ...
    c.pool(0).add_source(_make_tgfsr(7))
    assert c.reset() is True
...
def test_run_tempering_search_no_combo_done_when_no_result() -> None:
    ...
    c = ComboEnumerator(1, 32)
    c.pool(0).add_source(_make_tgfsr(5))
    assert c.reset() is True

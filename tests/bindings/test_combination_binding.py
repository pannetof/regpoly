...
"""Phase 2.4b-pre: pybind11 binding for Component + ComboEnumerator.
...
def test_combination_independent_pools_iterates_cartesian() -> None:
    from regpoly_cpp._regpoly_cpp import ComboEnumerator

    c = ComboEnumerator(2, 32)
    c.pool(0).add_source(_make_tgfsr(1))
    c.pool(0).add_source(_make_tgfsr(2))
    c.pool(1).add_source(_make_tgfsr(3))
    c.pool(1).add_source(_make_tgfsr(4))
...
def test_combination_shared_pool_enforces_cnk() -> None:
    ...
    c = ComboEnumerator(2, 32)
    for a in (1, 2, 3, 4):
        c.pool(0).add_source(_make_tgfsr(a))
...
def test_combination_kg_l_and_at() -> None:
    ...
    c = ComboEnumerator(1, 32)
    c.pool(0).add_source(_make_tgfsr(0xdeadbeef))
    assert c.reset() is True
    assert c.k_g == c[0].k()
    assert c.L > 0

...
#
# Snapshot refresh on 2026-05-24: ecart[2] flipped from 1 to 0 under
# the post-refactor `make_combined` / `CombinedF2LinearSource` shape.
# Verified externally with a NumPy GF(2) row-reduction on the same
# 63×62 submatrix: rank == 62 (full), so ecart[2] = 0 is correct
# under the standard convention. The old snapshot value (1) was
# locked from the pre-`make_combined` path and has no analytical
# backing.
_EXPECTED_ECART_S1 = (
    [-1]                                          # ecart[0] sentinel
    + [0, 0, 20, 14, 11, 9, 8, 6, 6, 5,           # ecart[1..10]
...
def test_combined_31_32_s1_ecart_locked():
    ...
    comb = make_combined(g31, g32, Lmax=64)

...
#
# Each cell sweeps (A, B, C) ∈ [1, w-1]³ and calls is_full_period() per
# candidate; for the (w=64, r ∈ {32, 64}) cells the per-candidate state
# is 2048/4096 bits and the inner loop has ~250 K iterations, well past
# the workspace's default `--timeout=120`. Bump these slow-lane tests
# to 30 min so the ci-slow lane completes instead of tripping
# pytest-timeout on the heavy cells.

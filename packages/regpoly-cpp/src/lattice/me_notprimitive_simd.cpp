...
struct SimdLinVec {
    ...
    SimdLinVec(const Generator& g, int lane_count_, int L_bits_,
               int sm, int wm)
        : gen(g.copy()),
          count(0), zero(false),
    ...
    // bit_pos.
    SimdLinVec(const Generator& g, int bit_pos,
...
int simd_pis_get_equidist(
    const Generator& warmed_gen, const BitVect& warmed_previous,
...
NTL::GF2X recover_chi_state_coord_bm(
    const std::vector<Generator*>& gens, int kg, int /*L*/)
{

...
// for each combination + retry runs the configured tests in order via
// the existing C++ runners (run_matricial_equidistribution,
// run_collision_free, run_tuplets), short-circuits when an
// equidistribution or tuplets test fails, and emits callbacks so
// Python can persist / display the result.
//

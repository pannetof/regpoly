...
TEST(EvalParamExpr, LiteralInt) {
    Params p;
    EXPECT_EQ(regpoly_random::eval_param_expr("42", p), 42);
    EXPECT_EQ(regpoly_random::eval_param_expr("-3", p), -3);
    EXPECT_EQ(regpoly_random::eval_param_expr("0", p), 0);
}
...
TEST(EvalParamExpr, LookupParam) {
    ...
    p.set_int("w", 32);
    p.set_int("k", 88);
    EXPECT_EQ(regpoly_random::eval_param_expr("w", p), 32);
    EXPECT_EQ(regpoly_random::eval_param_expr("k", p), 88);
    EXPECT_EQ(regpoly_random::eval_param_expr(" w ", p), 32);
}
...
TEST(EvalParamExpr, ParamPlusMinusN) {
    Params p;
    p.set_int("w", 32);
    EXPECT_EQ(regpoly_random::eval_param_expr("w+3", p), 35);
    EXPECT_EQ(regpoly_random::eval_param_expr("w-1", p), 31);
    EXPECT_EQ(regpoly_random::eval_param_expr("w + 3", p), 35);
}

TEST(EvalParamExpr, MissingParamThrows) {
    Params p;
    EXPECT_THROW(regpoly_random::eval_param_expr("bogus", p),
                 std::invalid_argument);
    EXPECT_THROW(regpoly_random::eval_param_expr("ghost+1", p),
                 std::invalid_argument);
}

TEST(SampleGeneric, Bitmask) {
    regpoly_random::seed(42);
    Params p;
    p.set_int("w", 8);
    auto spec = make_spec("a", "bitmask", "w");
    ASSERT_TRUE(regpoly_random::sample_generic_into(spec, p));
...
TEST(SampleGeneric, RangeRejectsBadArgs) {
    ...
    EXPECT_THROW(regpoly_random::sample_generic_into(bad, p),
                 std::invalid_argument);

    auto inverted = make_spec("m", "range", "10,1");
    EXPECT_THROW(regpoly_random::sample_generic_into(inverted, p),
                 std::invalid_argument);
}

TEST(SampleGeneric, PolyExponentsShape) {
    regpoly_random::seed(7);
    Params p;
    p.set_int("k", 32);
...
TEST(SampleGeneric, PolyExponentsRejectsSmallK) {
    ...
    p.set_int("k", 1);
    auto spec = make_spec("nocoeff", "poly_exponents", "k");
    EXPECT_THROW(regpoly_random::sample_generic_into(spec, p),
                 std::invalid_argument);
}

TEST(SampleGeneric, BitmaskVecMatchesLengthSourceShape) {
    regpoly_random::seed(11);
    Params p;
    p.set_int("w", 16);
...
TEST(SampleGeneric, UnknownRandTypeReturnsFalse) {
    ...
    EXPECT_FALSE(regpoly_random::sample_generic_into(spec, p));
}
...
TEST(SampleGeneric, NoneRandTypeReturnsFalse) {
    ...
    EXPECT_FALSE(regpoly_random::sample_generic_into(empty, p));
}

TEST(Seed, ProducesReproducibleSequence) {
    regpoly_random::seed(123);
    Params p;
    p.set_int("w", 16);
    auto spec = make_spec("a", "bitmask", "w");

...
class Paper:
    ...
    starred: bool = False
    deferred: bool = False    # cross-check / instantiation deferred —
                              # paper is listed for citation but has no
                              # ready-to-run generator entries yet
...
class Catalog:
    ...
    def reload_if_stale(self) -> None:
        if self.library_dir is None or not self.library_dir.is_dir():
            ...
            return
        on_disk = {
            p.stem: p for p in self.library_dir.glob("*.yaml")
            if not _is_cross_check_yaml(p)
        }
...
def _is_cross_check_yaml(path: Path) -> bool:
    """True when the file is an MTToolBox cross-check fixture
    (*_params.yaml) rather than a paper-organised library entry. These
    have a top-level `family:` field instead of `authors:` / `id:`."""
    return path.name.endswith("_params.yaml")
...
def _parse_paper(path: Path) -> Paper:
    ...
    generators = _parse_generators(
        raw.get("generators"), errors, allow_empty=deferred)
...
def _parse_generators(
    raw, errors: list[str], *, allow_empty: bool = False,
) -> list[Generator]:
    if raw is None or not isinstance(raw, list):
        if allow_empty and raw is None:
            return []
...
def _dry_build_generator(g: Generator, Lmax_default: int) -> None:
    ...
    for idx, comp in enumerate(g.components):
        try:
            ...
            g.errors.append(
                f"components[{idx}]: Generator.create({comp['family']}, "

...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # NOTE: family-name migration is intentionally *not* performed.
    # Existing rows can carry any of the historical names (TGFSR,
    # TGFSRGen, PolyLCG, MELG, dSFMTGen, …). The C++ factory and the
    # _FAMILY_ALIASES table both accept the old strings and resolve
    # them to the new canonical -Gen names, so old rows continue to
    # dispatch without rewriting the DB. New writes use the new names.
    # If a clean rewrite is ever wanted, expose it as an opt-in
    # `regpoly-web migrate-family-names` command rather than running
    # automatically on startup.
    #
    # The same opt-in pattern is used for the WELL matrices payload
    # migration (legacy `mat_types`/`mat_pi`/`mat_pu` flat triple →
    # paper-aligned `matrices: {T0..T7}` map). The migration is
    # JSON-payload-only, so SCHEMA_VERSION is unaffected. See
    # packages/regpoly-web/scripts/migrate_well_matrices.py.

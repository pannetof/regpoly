...
### DualLatticeBasisRow (polynomial vector = one basis row)
...
The C++ class was named `PolVect` pre-refactor; renamed to
`DualLatticeBasisRow` in the Phase 4 naming sweep.

### DualLatticeBase (the full basis)

| Field | Type | Meaning |
|-------|------|---------|
| `vect_[i]` | DualLatticeBasisRow | $i$-th basis vector $\mathbf{b}_i$ |

...
Let $f : \mathbb{F}_2^k \times \mathbb{F}_2^{d_1} \times \cdots \times \mathbb{F}_2^{d_n} \to \mathbb{F}_2^L$ be a function that is **linear over $\mathbb{F}_2$** in all its arguments jointly. Given a target output bit index $v$, determine which bits of each parameter vector $b_1, \ldots, b_n$ affect $y_v$, where $y = f(x, b_1, \ldots, b_n)$.
...
Linearity of $f$ gives $f(0, \ldots, 0) = 0$, and by the superposition principle each probe $f(0,\ldots,e_j,\ldots,0)$ returns exactly column $j$ of $B_k$. No assumption is needed about the internal structure of $f$ — only that the whole function is $\mathbb{F}_2$-linear.
...
the same algorithm applies without modification, provided the **composed function** is $\mathbb{F}_2$-linear overall. In that case the effective injection matrix for $b_k$ is:
...
where $w_i, w_{i+L}$ are words extracted from the generator state $x$, and $b \in \mathbb{F}_2^{64}$ is the tunable mask parameter. This is **linear in $b$ for fixed $x$**, with injection matrix $B(x) = \text{diag}(w_{i+L}(x))$. Consequently:

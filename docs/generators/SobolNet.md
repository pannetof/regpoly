...
**Aliases:** `sobol`, `Sobol`
**Base class:** `DigitalNet` → `F2LinearSource` → `ITestable`
...
`SobolNet` inherits from `DigitalNet`, which is a direct
`F2LinearSource` subclass — sibling of `Recurrence` under the
post-refactor hierarchy. This means the existing **equidistribution
machinery runs on a SobolNet without modification** — the matricial
kernel sees only the `ITestable` / `F2LinearSource` `init` / `next`
/ `get_output` interface, and `SobolNet` reinterprets those as
"set input index → advance coordinate counter →
return $C_j \cdot \mathbf{i}$".
...
# Existing equidistribution test runs unchanged on a bare F2LinearSource:
eqt = EquidistributionTest(L=4, delta=[sys.maxsize] * 5, mse=sys.maxsize)
print(eqt.run(gen).display_table(gen, by="l"))

# t-value test:
tvt = TValueTest(s_max=4, max_t_sum=sys.maxsize)
print(tvt.run(gen).display())
```

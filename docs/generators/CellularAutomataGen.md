...
components are combined via the existing `CombinedF2LinearSource`
wrapper to form the full paper-style generator (no new combined-family
class needed).
...
extracts it lazily via the base `Recurrence::transition_matrix()` helper.

### Characteristic polynomial

Recovered at runtime by the inherited Berlekamp–Massey routine
(`Recurrence::char_poly()`).  For an isolated CA at $s = 1$ the resulting
...
- **Combined two-component PRNGs** are built via
  `CombinedF2LinearSource` with two `CellularAutomataGen` components.
  Each component carries its own time-spacing s; the combined output
  is the bitwise XOR.

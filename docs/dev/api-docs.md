...
def analyze_single_generator(gen, *, L=32, method="matricial"):
    ...
    See Also
    --------
    :cpp:func:`regpoly::core::test_me_matricial` :
...
| C++ class                       | `` :cpp:class:`regpoly::core::Recurrence` `` |
| C++ free function               | `` :cpp:func:`regpoly::core::test_me_matricial` `` |
...
- `@code{.cpp}` block on every class a downstream user would
  construct directly — every `*Gen` / `*Net` family,
  `CombinedF2LinearSource`, `ComboEnumerator`, `Catalog`,
  `TemperingOptimizer`, etc.
...
/// @see :cpp:class:`regpoly::core::ComboEnumerator`   — sibling C++ type.
```

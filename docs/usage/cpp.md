...
The pre-v2 `.dat` parameter format is **no longer supported**; convert
any such fixtures to the YAML catalog format up-front.
...
`<regpoly/regpoly.h>`. It re-exports the abstract source hierarchy
(`ITestable`, `F2LinearSource`, `Recurrence`), the composition
wrapper `CombinedF2LinearSource`, the `Transformation` base + chain,
the `ComboEnumerator` / `F2LinearSourcePool` search-loop iterator,
`BitVect`, and the lattice / equidistribution runners (`test_me_lat`,
`test_me_harase`, `test_me_notprimitive`, `test_me_notprimitive_simd`,
`test_me_matricial`, `run_collision_free`). The catalog
(`<regpoly/catalog.h>`) and the YAML config loader
(`<regpoly/seek_config.h>`) are separate public headers. Concrete
generator families are not part of the public ABI — build them via
the catalog or the search config loader rather than directly.
...
int main(int argc, char* argv[]) {
    ...
    // 1. Load the YAML search config and build the runtime ComboEnumerator.
    auto cfg   = yc::load_seek_config(yaml_path);
    auto built = yc::build_search(cfg);
    ComboEnumerator& comb = *built.combination;

    // 2. Snapshot the active combination — materialise per-component
    //    tempering chains into a single CombinedF2LinearSource we can
    //    iterate. Returned as `unique_ptr<ITestable>` because lattice-
    //    family kernels accept `ITestable&` polymorphically.
...
The pre-v2 `.dat` parameter format was retired in v2.x and is no longer
supported by any layer; convert any `.dat` fixture to the YAML catalog
format up-front.

...
// Pybind11 registration for Recurrence subclass types.
//
// The `py::class_<Cls, Recurrence>(m, name)` instantiations are
...
// One-line helper for the common case (subclass of Recurrence).
template <typename Cls>
void bind_gen(py::module_& m, const char* name) {
    py::class_<Cls, Recurrence, std::unique_ptr<Cls>>(m, name);
}
...
void register_generator_types(py::module_& m) {
    ...
    py::class_<F2wBaseGen, Recurrence, std::unique_ptr<F2wBaseGen>>(m, "F2wBaseGen");
    ...
    py::class_<WELLGen, Recurrence, std::unique_ptr<WELLGen>>(m, "WELLGen")

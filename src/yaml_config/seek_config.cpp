...
// Apply one YAML-derived value into a ParamBag, mirroring the Python
// loader's lenient int / string / list coercion.
void apply_yaml_to_params(ParamBag& dst,
...
ComponentSpec parse_component(const YAML::Node& comp_node,
    ...
    if (gens_node.IsScalar()) {
        ...
        if (gens_node["legacy_file"]) {
            throw std::runtime_error(filename + ": components["
                + std::to_string(comp_index)
                + "].generators.legacy_file: the pre-v2 .dat parameter "
                  "source is no longer supported. Re-author this config "
                  "with an inline 'family:' block.");
...
SeekTestSpec parse_test(const YAML::Node& tn,
    ...
    if (type == "equidistribution") {
        std::string method = tn["method"]
            ...
            : "matricial";
        // Method-name validation lives in EquidistributionMethodRegistry; the YAML
        // parser no longer owns the list.
        if (!EquidistributionMethodRegistry::has(method)) {
            throw std::runtime_error(filename + ": tests["
...
// Build a Recurrence pool for a single inline component spec — calls
// create_generator(family, merged_params, Lmax) for every entry.
std::vector<std::unique_ptr<Recurrence>>
build_inline_pool(const ComponentSpec& spec, int Lmax,
                  const std::string& context) {
    std::vector<std::unique_ptr<Recurrence>> out;
...
// to a tempering ParamBag block when 'w' is missing. Mirrors catalog
// .cpp's dry_build_generator.
void apply_w_default(ParamBag& tp, const ComponentSpec& comp) {
    if (tp.has("w")) return;
...
SeekBuild build_search(const SeekConfig& cfg) {
    ...
    built.combination = std::make_unique<ComboEnumerator>(J, cfg.Lmax);
    ...
    for (int j = 0; j < J; ++j) {
        ...
        if (spec.source == ComponentSpec::Source::Same) {
            ...
            built.combination->pool(j).copy_pool_from(
                built.combination->pool(j - 1));
        } else {
            std::vector<std::unique_ptr<Recurrence>> pool =
            ...
            for (const auto& g : pool) {
                built.combination->pool(j).add_source(*g);
            }
        ...
        for (size_t ti = 0; ti < spec.tempering.size(); ++ti) {
            ...
            built.combination->pool(j).add_trans(*t);
    ...
    if (!built.combination->reset()) {
        throw std::runtime_error(
            "build_search: ComboEnumerator::reset() returned false — "

...
public:
    ...
    /**
     * @brief Build the generator matrix from generators.
     *
     * Phase 2 Step 6b: each component carries its tempering chain
     * intrinsically on its `F2LinearSource::tempering_` member; the
     * kernel reads tempered bits via `tempered_output()` and no longer
     * receives chains as a separate argument.
     *
     * @param gens         Component generators (chains already installed
     *                     via `set_tempering()` if any).

 ...
 * @ingroup core
 */
class LaggedTemperingTrans : public Transformation {
public:
    /**
     * @brief Construct a LaggedTemperingTrans with explicit parameters.
     *
     ...
     */
    LaggedTemperingTrans(int w, int sigma, int L, uint64_t b);

    /**
     * @brief Factory hook — build from a ParamBag dict.
     * @param params  Parameter dict with keys `w, sigma, L, b`.
     * @return        Owning pointer to the new LaggedTemperingTrans.
     * @throws std::runtime_error  If a required parameter is missing.
     */
    static std::unique_ptr<Transformation> from_params(const ParamBag& params);
     ...
     */
    void update(const ParamBag& params) override;

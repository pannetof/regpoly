 ...
 * @ingroup core
 *
 * `TemperingOptimizerCache` — dual lattice with StackBase for tempering
 * optimisation.
 *
...
private:
    // Non-owning pointer to the CombinedF2LinearSource passed to the
    // ctor. `rebuild()` / `step()` need to re-call `find_polys(*cs_,
    // ...)` after a bitmask perturbation; the source must outlive the
    // cache.
    const CombinedF2LinearSource* cs_;

 ...
 * @ingroup core
 */
class MarsaXorshiftGen : public Recurrence {
public:
    ...
    /**
     * @brief Build a MarsaXorshiftGen from a ParamBag dict (registry factory hook).
     * @param params  Parameter dict with the type-specific keys (see class docs).
     * @param L       Output resolution in bits.
     * @return        A constructed MarsaXorshiftGen as a polymorphic Recurrence pointer.
     ...
     *
     * @param resolved  ParamBag with `type` and `w` pinned.
     * @param L         Output resolution in bits.

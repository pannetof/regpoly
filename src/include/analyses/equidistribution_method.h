 ...
 *  2. Register it via `EquidistributionMethodRegistry::reg("baz", ...)` in
 *     `factory.cpp`.
 *
 ...
 * constructed fresh per dispatch via `EquidistributionMethodRegistry`.
 *
 * @ingroup core
 */
...
public:
     ...
     *
     * @param gen   Source object to evaluate. Accepted shapes: any
     *              `Recurrence` (PRNG), `CombinedF2LinearSource` (legacy
     *              inheritance wrapper), or `CombinedF2LinearSource`
     *              (composition wrapper). Lattice-family methods throw
     *              if any component is not a `Recurrence` (e.g. a
     *              `DigitalNet`); the matricial method handles any
     *              `ITestable` polymorphically.
 ...
 * @ingroup core
 */
class EquidistributionMethodRegistry {
public:

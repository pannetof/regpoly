 ...
 * pure-C++ loop that drives a `ComboEnumerator` and fires Python
 ...
 *
 *   - `ComboEnumerator` iteration (`reset` / `next`),
...
using TempSearchOnComboStartFn =
    std::function<void(ComboEnumerator&, int /*combo_idx*/)>;
/// Per-try callback. Fires once per try inside each combo; must return the try's outcome.
using TempSearchOnTryFn =
    std::function<TemperingTryResult(
        ComboEnumerator&, int /*combo_idx*/, int /*try_idx*/, bool /*is_first*/)>;
...
/**
 * @brief Run the tempering-search loop over a configured `ComboEnumerator`.
 *

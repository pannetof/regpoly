 ...
 * `Recurrence` output. Each step transforms an output `BitVect` in
 ...
 * on top of a `Recurrence`'s output word. They are owned by
 * `Component` (via `unique_ptr`) and assembled into per-component
 * tempering chains by `ComboEnumerator`. The optimiser explores the
...
public:
     ...
     * `build_combined_from_enumerator()` to keep candidate chains
    ...
    /**
     * @brief Re-parameterise the step from a `ParamBag` bundle.
     *
     ...
     */
    virtual void update(const ParamBag& params) = 0;

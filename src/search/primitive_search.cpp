...
void merge_into(ParamBag& dst, const ParamBag& src) {
    ...
    const OnProgressFn& on_progress)
{
    ...
    while (true) {
        ...
        // ParamBag with a freshly-sampled cost-bounded `matrices` BEFORE
        ...
        try {
            ...
            // PrimitiveSearch.run()'s try/except around Recurrence.create.
            continue;
        }

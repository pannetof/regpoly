...
LaggedTemperingTrans::LaggedTemperingTrans(int w, int sigma, int L, uint64_t b)
    : sigma_(sigma), L_(L), b_(b)
{
    w_ = w;
}

std::string LaggedTemperingTrans::name() const {
    return "Lagged Tempering";
}

std::string LaggedTemperingTrans::display_str() const {
...
void LaggedTemperingTrans::apply(BitVect& state) const {
    int n = state.nbits();
...
std::unique_ptr<Transformation> LaggedTemperingTrans::copy() const {
    return std::make_unique<LaggedTemperingTrans>(w_, sigma_, L_, b_);
}

void LaggedTemperingTrans::update(const ParamBag& params) {
...
std::unique_ptr<Transformation> LaggedTemperingTrans::from_params(const ParamBag& params) {
    ...
    return std::make_unique<LaggedTemperingTrans>(w, sigma, L, b);
}

std::vector<ParamSpec> LaggedTemperingTrans::param_specs() {
    return {

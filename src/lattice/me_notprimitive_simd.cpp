...
PhiPick select_phi(const NTL::GF2X& chi) {
    ...
    throw std::runtime_error(
        "test_me_notprimitive_simd: chi has no nontrivial irreducible factor");
}

// step_once / apply_polynomial used to build V-projected seeds.
//
// step_once via init+next applies one full f-step.  CRITICAL: SFMTGen's
...
BitVect step_once(const Recurrence& proto, const BitVect& r) {
    auto g = proto.clone_recurrence();
    g->set_raw_state(r);
    ...
    // N words per step rather than N+1.  SFMTGen and any future
    ...
    // SFMTGen path: replicate generate_all via N per-word advances,
    ...
    // Cleaner: just advance N times — for SFMTGen, simd_advance_one_word
...
struct SimdLinVec {
    ...
    SimdLinVec(const Recurrence& g, int lane_count_, int L_bits_,
               int sm, int wm)
        : gen(g.clone_recurrence()),
          count(0), zero(false),
    ...
    // bit_pos.
    SimdLinVec(const Recurrence& g, int bit_pos,
    ...
    // GF(2) addition: XOR generator state (index-aligned via SFMTGen
...
int simd_pis_get_equidist(
    const Recurrence& warmed_gen, const BitVect& warmed_previous,
...
// primitive component of SFMTGen's char poly — the non-V dim-31 subspace
...
NTL::GF2X recover_chi_state_coord_bm(
    const std::vector<Recurrence*>& gens, int kg, int /*L*/)
{
...
// ─────────────────────────────────────────────────────────────────────
// Public entry point: test_me_notprimitive_simd.
...
{
    auto gens = cs.recurrence_components("test_me_notprimitive_simd");
    int lane_count = 1;
    for (auto* g : gens) {
    ...
    if (lane_count == 1) {
        BitVect chi_bv = polychar_comb(cs);
        NTL::GF2X chi;
        NTL::SetCoeff(chi, kg);
        for (int j = 0; j < kg; j++)
    ...
    if (gens.size() != 1) {
        throw std::runtime_error(
            "test_me_notprimitive_simd: multi-component SIMD configurations "
            "are not yet supported.  Use single-component SFMTGen/dSFMT/MTGPGen.");
    }
    ...
    // the primitive factor, as it does for SFMTGen).  The combination
    ...
    // where quotient = chi/primitive.  For SFMTGen, the per-word transition
    // matrix M_pw has min poly = primitive (degree mexp = p); the char
    // poly factors as primitive * (degree-(kg-p) factor) where (kg-p) =
    // 31 for SFMTGen-19937, 33 for SFMTGen-607.  Computing the explicit
    ...
    // for the non-primitive factor; for SFMTGen this is typical: the
    ...
    if (seed_v.nbits() == 0) {
        throw std::runtime_error(
            "test_me_notprimitive_simd: V-nonzero seed construction failed");
    }
    ...
    for (int sm = 0; sm < lane_count; sm++) {
        ...
        for (int wm = 1; wm <= lane_count; wm++) {
            ...
            // post-setWeightMode).
            auto warmed_gen = gens[0]->clone_recurrence();
    ...
    res.se = 0;
    // Phase 2 step 7 (Q6): SIMD-PIS handles the non-primitive case via
    // Saito-Matsumoto 2008 reduction; result is certified by construction.
    res.verified = true;
    for (int v = 1; v <= RES; v++) {
        int upper = p / v;

...
#include "me_harase.h"
#include "me_helpers.h"  // polychar_comb, EquidistributionResult
#include <climits>
#include <algorithm>
#include <cstring>
...
// operations — Recurrence is NOT modified.
//
...
struct LatticeVec {
    // J component generators (each carries its own intrinsic tempering chain
    // via F2LinearSource::tempering_; reading bits via get_output() composes).
    std::vector<std::unique_ptr<Recurrence>> gens;
    int J;
    ...
    LatticeVec deep_copy() const {
        LatticeVec v;
        v.J = J;
        for (auto& g : gens) v.gens.push_back(g->clone_recurrence());
        v.next = next;
        v.count = count;
        v.zero = zero;
        return v;
    ...
    static LatticeVec from_generators(const std::vector<Recurrence*>& tpl) {
        LatticeVec v;
        v.J = (int)tpl.size();
        for (auto* g : tpl) v.gens.push_back(g->clone_recurrence());
        v.next = 0;
        v.count = 0;
        v.zero = false;
        return v;
...
private:
    ...
    // Phase 2 step 6: chain on source — `tempered_output()` composes
    // raw_output + chain in one call. `trans` member kept for API
    // compatibility (still passed in by the legacy caller) but unused.
...
{
    auto gens = cs.recurrence_components("test_me_harase");
    int RES = std::min(maxL, kg);
    int w = L;  // output word width (typically 64)

    EquidistributionResult result;
    result.ecart.resize(maxL + 1, -1);
    result.se = 0;
    // Phase 2 step 7 (Q6): lattice precondition (primitive chi) implies
    ...
    for (int i = 0; i < v; i++) {
        basis[i] = LatticeVec::from_generators(gens);
        basis[i].set_zero();
        basis[i].count = 0;
        basis[i].zero = false;
    ...
    basis[v] = LatticeVec::from_generators(gens);
    {
...
int compute_kv(
    ...
    int kg, int v)
{
    auto gens = cs.recurrence_components("compute_kv");
    ...
    // Recurrence vector: canonical init, advance one step
    basis[v] = LatticeVec::from_generators(gens);
    {
        int pos = 0;
        for (int j = 0; j < basis[v].J; j++) {
...
// ══════════════════════════════════════════════════════════════════════════
// HaraseRankCache — StackBase strategy for tempering optimization
//
...
struct HaraseRankCache::Impl {
    std::vector<Recurrence*> gens;
    int kg;
    int L;
        ...
        while (!basis[bit_len].zero) {
            ...
            if (++iters > max_iters) {
                throw std::runtime_error(
                    "HaraseRankCache::reduce_at did not converge after "
...
HaraseRankCache::HaraseRankCache(
    const CombinedF2LinearSource& cs,
    int kg, int L)
    : gens_(cs.recurrence_components("HaraseRankCache")),
      kg_(kg), L_(L),
...
std::vector<int> HaraseRankCache::compute_all() {
    auto& I = *impl_;
    I.init_basis();
...
int HaraseRankCache::restore_and_reduce(int v) {
    auto& I = *impl_;

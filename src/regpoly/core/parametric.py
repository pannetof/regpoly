...
`NotEnumerable`, and `build_parameter_space_enumerator()` — a thin shim over the
C++ registry.
"""
...
class Enumerator:
    """Python-side wrapper around the C++ ``ParameterSpaceEnumerator``.
...
def build_parameter_space_enumerator(
    ...
    import regpoly._regpoly_cpp as _cpp
    try:
        cpp = _cpp.make_parameter_space_enumerator(family, resolved, L)
    except Exception as exc:

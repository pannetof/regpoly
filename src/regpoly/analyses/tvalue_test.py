...
class TValueTest(AbstractTest):
    ...
    def run(self, gen_or_C, *args, **kwargs) -> TValueResults:
        """Execute the t-value test.

        Accepts a Generator (any subclass — e.g. SobolNet,
        NiederreiterF2Net, CombinedF2LinearSource) or — during the Phase-1
        migration window — a Python Combination.

        For ``method = METHOD_NIEDERREITER_PIRSIC`` the underlying C++
        kernel raises; the exception propagates so callers learn

...
std::unique_ptr<ITestable> SFMTGen::copy() const {
...
void SFMTGen::simd_add_state(const Recurrence& other) {
...
BitVect SFMTGen::raw_output() const {
    BitVect out(32);
    out.set_word(0, 32, (uint64_t)last_output_);
    return out;
}

std::unique_ptr<Recurrence> SFMTGen::from_params(

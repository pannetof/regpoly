...
#include "parameter_space_enumerator.h"
...
TauswortheGen::TauswortheGen(int k, const std::vector<int>& Q, int s, bool quicktaus, int L)
    : Recurrence(k, L), Q_(Q), NbCoeff_((int)Q.size()),
      s_(s), quicktaus_(quicktaus), gen_kms_(k - s)
{
    state_ = BitVect(L);
}

std::string TauswortheGen::name() const { return "TauswortheGen Generator"; }

std::string TauswortheGen::display_str() const {
...
void TauswortheGen::init(const BitVect& init_bv) {
    state_ = BitVect(L_);
...
std::unique_ptr<ITestable> TauswortheGen::copy() const {
    auto p = std::make_unique<TauswortheGen>(k_, Q_, s_, quicktaus_, L_);
    p->state_ = state_.copy();
    p->gen_kms_ = gen_kms_;
    return p;
}

BitVect TauswortheGen::char_poly() const {
...
void TauswortheGen::get_transition_state(uint64_t* out_words, int out_nwords) const {
    BitVect tmp(k_);
...
void TauswortheGen::next_general() {
...
std::unique_ptr<Recurrence> TauswortheGen::from_params(const ParamBag& params, int L) {
    ...
    if (params.has("poly")) {
        ...
        if (Q.empty())
            throw std::invalid_argument("TauswortheGen: poly is empty");
        k = Q.back();
        ...
        if (params.has("k")) {
            ...
            if (k_user > 0 && k_user != k) {
                throw std::invalid_argument(
                    "TauswortheGen: k=" + std::to_string(k_user) +
        ...
        if (params.has("nb_terms")) {
            ...
            if (nbt_user > 0 && nbt_user != (int)Q.size()) {
                throw std::invalid_argument(
                    "TauswortheGen: nb_terms=" + std::to_string(nbt_user) +
        ...
        if (k <= 0)
            throw std::invalid_argument(
                "TauswortheGen: supply `poly`, or `k` (nb_terms optional, "
                "defaults to 3)");
        int nb_terms = 3;
        if (params.has("nb_terms")) {
        ...
        Q = TauswortheGen::random_poly(k, nb_terms, quicktaus, L, s_hint);
    }
    ...
    if (!TauswortheGen::is_admissible(Q, s, quicktaus, L))
        throw std::runtime_error(
            "TauswortheGen polynomial violates the quicktaus "
            "admissibility inequality (0 < s <= k - q_{t-2} < k <= L)");

    return std::make_unique<TauswortheGen>(k, Q, s, quicktaus, L);
}

std::vector<ParamSpec> TauswortheGen::param_specs() {
...
bool TauswortheGen::is_admissible(
...
// TauswortheGen::generate_random.
static int pick_admissible_s(int k, int s_max, std::mt19937_64& rng) {
    if (s_max < 1)
        throw std::invalid_argument(
            "TauswortheGen::generate_random: no admissible s (s_max < 1)");
    ...
    throw std::invalid_argument(
        "TauswortheGen::generate_random: could not find s in [1, s_max] "
        "coprime with 2^k - 1 after 256 tries");
}

RandomParamResult TauswortheGen::generate_random(
    const std::string& rand_type,
    const std::string& /*rand_args*/,
    const ParamBag& params, int L)
{
    ...
    if (rand_type == "tausworthe_s") {
        ...
        if (params.get_int("k", 0) > 0) {
            ...
            throw std::invalid_argument(
                "TauswortheGen: randomizing s needs k or poly");
        }
        ...
        if (quicktaus && has_poly) {
            ...
            if (q_top == 0)
                throw std::invalid_argument(
                    "TauswortheGen: poly has no interior exponents");
            s_max = k - q_top;
        } else if (quicktaus) {
    ...
    if (rand_type == "tausworthe_poly") {
        ...
        if (k <= 0)
            throw std::invalid_argument(
                "TauswortheGen: randomizing poly needs k > 0");
        int nb_terms = (int)params.get_int("nb_terms", 0);
        if (nb_terms <= 0) nb_terms = 3;
        const bool quicktaus = params.get_bool("quicktaus", true);
        ...
        out.is_vec = true;
        out.vec_val = TauswortheGen::random_poly(
            k, nb_terms, quicktaus, L, s);
        return out;
    }

    throw std::invalid_argument(
        "TauswortheGen::generate_random: unsupported rand_type '"
        + rand_type + "'");
}

std::vector<int> TauswortheGen::random_poly(
    int k, int nb_terms, bool quicktaus, int L, int s)
{
    if (nb_terms < 3)
        throw std::invalid_argument(
            "TauswortheGen::random_poly: nb_terms must be >= 3");
    if (nb_terms % 2 == 0)
        throw std::invalid_argument(
            "TauswortheGen::random_poly: nb_terms must be odd");
    if (k <= 0)
        throw std::invalid_argument(
            "TauswortheGen::random_poly: k must be positive");
    if (nb_terms - 2 > k - 1)
        throw std::invalid_argument(
            "TauswortheGen::random_poly: nb_terms too large for k");

    if (quicktaus && k > L)
        throw std::invalid_argument(
            "TauswortheGen::random_poly: quicktaus requires k <= L");
    ...
    if (quicktaus) {
        ...
        if (q_max < t_interior)
            throw std::invalid_argument(
                "TauswortheGen::random_poly: nb_terms too large for the "
...
// Concrete enumerator for TauswortheGen (s × poly).  Enumeration order:
//   outer axis = s    (varies slowest)
//   inner axis = poly (q_top, then unrank the interior subset)
class TausworthePolyEnumerator : public ParameterSpaceEnumerator {
public:
    ...
    ParamBag at(const std::string& idx_dec) const override {
...
std::unique_ptr<ParameterSpaceEnumerator> TauswortheGen::make_enumerator(
    const ParamBag& resolved, int L)
{
    if (!resolved.has("k"))

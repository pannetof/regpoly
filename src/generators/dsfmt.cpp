...
DSFMTGen::DSFMTGen(int mexp, int pos1, int sl1,
    ...
    : Recurrence(128 * ((mexp - 128) / 104 + 2), L),
      mexp_(mexp),
      N_((mexp - 128) / 104 + 1),
      pos1_(pos1), sl1_(sl1),
...
std::unique_ptr<ITestable> DSFMTGen::copy() const {
...
BitVect DSFMTGen::raw_output() const {
...
void DSFMTGen::simd_add_state(const Recurrence& other) {
...
std::unique_ptr<Recurrence> DSFMTGen::from_params(

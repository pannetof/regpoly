...
NiederreiterF2Net::NiederreiterF2Net(int m, int s_max)
    : DigitalNet(m, s_max) {
    ...
    if (m > 30) {
        throw std::invalid_argument(
            "NiederreiterF2Net: m > 30 not supported in v1 (got "
...
void NiederreiterF2Net::enumerate_irreducibles(int s_max) {
...
void NiederreiterF2Net::build_matrices(int m, int s_max) {
    ...
    for (int j = 1; j <= s_max; ++j) {
        ...
        if (r_max * ej > 63) {
            ...
            oss << "NiederreiterF2Net: polynomial degree " << (r_max * ej)
...
const std::vector<BitVect>& NiederreiterF2Net::generating_matrix(int j) const {
    return (*matrices_)[j - 1];
}

std::string NiederreiterF2Net::display_str() const {
    std::ostringstream oss;
    oss << "NiederreiterF2Net(m=" << m() << ", s_max=" << s_max() << ")";
    return oss.str();
}

std::unique_ptr<ITestable> NiederreiterF2Net::copy() const {
    return std::make_unique<NiederreiterF2Net>(*this);
}

...
When compiled with Cython, the @cython.cclass decorator produces a C-level
extension type with fast attribute access.  Without Cython the decorators
are no-ops and the module runs as pure Python.
"""
...
class BitVect:
    ...
    n: object = cython.declare(object, visibility='public')
    _val: object = cython.declare(object, visibility='public')

    # ------------------------------------------------------------------ #
    #  Construction / factory methods                                      #
    # ------------------------------------------------------------------ #

    def __init__(self, n: int, val: object = 0):
        """
    ...
    @cython.ccall
    def put_to_zero(self) -> cython.void:
        """PutBVToZero: set all bits to 0 in place."""
        self._val = 0

    @cython.ccall
    def put_to_all_ones(self) -> cython.void:
        """AllOnes (in place): set all bits to 1."""
        self._val = (1 << self.n) - 1

    @cython.ccall
    def set_canonic(self, l: int) -> cython.void:
    ...
    @cython.ccall
    def set_mask(self, l: int) -> cython.void:
    ...
    @cython.ccall
    def set_invmask(self, l: int) -> cython.void:
    ...
    @cython.ccall
    def put_bit(self, i: int, v: int) -> cython.void:
    ...
    @cython.ccall
    def iand_mask(self, t: int) -> cython.void:
    ...
    @cython.ccall
    def iand_invmask(self, t: int) -> cython.void:
    ...
    @cython.ccall
    def inverse_self(self) -> cython.void:
    ...
    @cython.ccall
    def lshift1_self(self) -> cython.void:
        """
    ...
    def lrot_shift(self, k: int, w: int) -> "BitVect":
        ...
        top: object = self._val >> (self.n - w)
        bottom: object = self._val & ((1 << (self.n - w)) - 1)
        rotated: object = ((top << k) | (top >> (w - k))) & ((1 << w) - 1)
        bv: BitVect = BitVect.__new__(BitVect)
        bv.n = self.n

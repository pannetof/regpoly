...
// side that translates a SeekConfig into a ComboEnumerator via the existing
// factories.
...
TEST(SeekConfig, RejectsLegacyFileSourceAsUnsupported) {
    // The pre-v2 `legacy_file:` component source is rejected at the
    // YAML loader. The error message must flag it as no longer supported.
    ...
    try {
        ...
        EXPECT_NE(msg.find("legacy_file"), std::string::npos)
            << "error message should name the rejected legacy_file key; got: "
            << msg;
        EXPECT_NE(msg.find("no longer supported"), std::string::npos)
            << "error message should flag legacy_file as unsupported; got: "
            << msg;
    }
...
TEST(SeekConfig, ParsesSameSentinelAsSharedPool) {
    ...
    ASSERT_NE(built.combination, nullptr);
    EXPECT_EQ(built.combination->J(), 2);
    // F2LinearSourcePool 1 shares pool — pool_id should match component 0's.
    EXPECT_EQ(built.combination->pool(0).pool_id(),
              built.combination->pool(1).pool_id());
    EXPECT_FALSE(built.combination->exhausted());
}
...
TEST(SeekConfig, BuiltSearchRunsSeekDriver) {
    ...
    auto on_iter = [&](ComboEnumerator&, const SeekIterResult&) { ++selections; };

...
// Build a 1-component CombinedF2LinearSource + tempMK chain so the cache
...
struct Fixture {
    ...
    std::unique_ptr<CombinedF2LinearSource> cs;
    std::unique_ptr<TemperingOptimizerCache> cache;
    ...
    static Fixture make() {
        ...
        // Phase 2+ (Q1): tempering chain lives on the source intrinsically.
        // Install it via set_tempering() before constructing the cache.
        // Keep an aliased raw pointer in `f.trans` (for ParamLocator below)
        // by reading it back from the source's chain.
        {
            TemperingChain chain;
            chain.add(std::move(f.trans));
            f.gen->set_tempering(std::move(chain));
            f.trans = nullptr;  // ownership moved to source
        }

        // Build the cache: wrap the single source in a CombinedF2LinearSource.
        std::vector<std::unique_ptr<F2LinearSource>> comps;
        comps.push_back(f.gen->clone_source());
        f.cs = std::make_unique<CombinedF2LinearSource>(std::move(comps), 32);
        f.cache = std::make_unique<TemperingOptimizerCache>(
            *f.cs, /*kg=*/31, /*L=*/32);
        f.L = 32;

        // ParamLocator for `b`. Width = 32 (tempMK's w). The chain step
        // lives on the source's intrinsic chain — fetch the raw pointer
        // by reading back from `gen->tempering().at(0)`.
        TemperParamLocator loc;
        loc.trans = &f.gen->tempering().at(0);
        loc.param_name = "b";
        loc.width = 32;

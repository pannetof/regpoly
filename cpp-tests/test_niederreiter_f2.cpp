...
//
// Phase 3: NiederreiterF2Net — irreducible enumeration and Laurent
// matrix construction.
//
...
using regpoly::core::NiederreiterF2Net;
using regpoly::core::test_me_matricial;

TEST(NiederreiterF2, ConstructorRejectsExcessiveM) {
    EXPECT_THROW(NiederreiterF2Net(/*m=*/31, /*s_max=*/31), std::invalid_argument);
}

TEST(NiederreiterF2, ConstructorRejectsSMaxLessThanM) {
    EXPECT_THROW(NiederreiterF2Net(/*m=*/4, /*s_max=*/3), std::invalid_argument);
}

TEST(NiederreiterF2, IrreducibleEnumerationMatchesCanonical) {
    NiederreiterF2Net net(4, 8);
...
TEST(NiederreiterF2, M2S2FirstCoordinateIsIdentity) {
    ...
    NiederreiterF2Net net(/*m=*/2, /*s_max=*/2);
...
TEST(NiederreiterF2, MatricialEquidistributionRunsUnchanged) {
    ...
    const int m = 4;
    NiederreiterF2Net net(m, /*s_max=*/m);
...
TEST(NiederreiterF2, CopyClonesSharedMatrices) {
    NiederreiterF2Net net(4, 4);
    auto clone = net.copy();
    EXPECT_NE(clone.get(), &net);
    EXPECT_EQ(clone->name(), "NiederreiterF2Net");
}

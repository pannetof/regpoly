...
// Tests for Recurrence::default_test_method(test_type).
//
// Coverage matrix:
...
//   - CombinedF2LinearSource overrides and returns "notprimitive" for
...
// JEquals1 invariant: a CombinedF2LinearSource wrapping a single component
...
TEST(EquidistributionMethodRegistry, RegisteringNewMethodWorksWithoutDispatchChanges) {
    // Pre: method is unknown.
    EXPECT_FALSE(EquidistributionMethodRegistry::has("fake_test_method"));

    EquidistributionMethodRegistry::reg("fake_test_method",
        []{ return std::unique_ptr<EquidistributionMethod>(new FakeMethod); });

    // Post: registry knows the new method.
    EXPECT_TRUE(EquidistributionMethodRegistry::has("fake_test_method"));

    // The factory builds an instance and run() returns FakeMethod's
    // distinctive result — no edits to seek_search.cpp, no enum entry,
    // no dispatch branch.
    auto m = EquidistributionMethodRegistry::create("fake_test_method");

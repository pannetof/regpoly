...
gen_file1 [...]``) and the ``.dat`` parameter format are no longer
supported; author a ``.yaml`` config instead.
"""
...
def main() -> None:
    ...
    if not (arg.endswith(".yaml") or arg.endswith(".yml")):
        print(
            ...
            f"(nb_comp test_file gen_file1 ...) and the .dat format are no "
            f"longer supported; author a .yaml config instead.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if "family" in search:
        ...
        print(f"  k_g = {comb.k()}, L = {comb.L()}, J = {comb.J()}")
        if results:

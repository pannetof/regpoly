...
def load_tested_generator(filepath: str) -> tuple:
    """
    Load a tested generator YAML file.

    Returns (combined, results_dict) where `combined` is a
    `_cpp.CombinedF2LinearSource` ready to feed to any `AbstractTest.run`.
    """
...
def _single_component_data(comb, j: int) -> dict:
    gen = comb[j]
...
def _multi_component_data(comb) -> dict:
    components = []
...
def _load_single(data: dict):
    ...
    for t_data in data.get("tempering", []):
        ...
        trans_list.append(Transformation.create(trans_type, **t_params))

    return make_combined(gen, trans=trans_list, Lmax=L)


def _load_multi(data: dict):
    comp_list = data["components"]
    Lmax = max(entry["generator"]["L"] for entry in comp_list)
    ...
    for entry in comp_list:
        ...
        gens.append(Generator.create(family, L, **params))

        chain = []
        for t_data in entry.get("tempering", []):
            trans_type = t_data["type"]
            t_params = {k: v for k, v in t_data.items() if k != "type"}
            chain.append(Transformation.create(trans_type, **t_params))
        trans_chains.append(chain)

    return make_combined(*gens, trans=trans_chains, Lmax=Lmax)

...
>>> cat = Catalog()                              # bundled package catalog
...
# Sentinel distinguishing "no argument given" (→ bundled catalog) from an
# explicit ``None`` (→ empty catalog, for tests that replace ``_impl``).
_BUNDLED = object()
...
class Catalog:
    ...
    :cpp:class:`regpoly::library::Catalog` : the C++ implementation this wraps
        (header: ``packages/regpoly/src/include/library/catalog.h``).
    """

    def __init__(self, library_dir: Optional[Path | str] = _BUNDLED) -> None:
        ...
        ----------
        library_dir
            Path to the catalog directory (containing paper YAML files).
            Either a `str`, a `pathlib.Path`, or `None`. **Omit the
            argument** to use the catalog bundled in the package
            (:func:`bundled_library_dir`). Pass `None` to construct the
            underlying C++ catalog with an empty path (it will fail any
            subsequent `load()`) — use this only for tests that replace
            `self._impl` afterwards.
        """
        if library_dir is _BUNDLED:
            library_dir = bundled_library_dir()

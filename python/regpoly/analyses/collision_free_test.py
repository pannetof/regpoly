...
class CollisionFreeTest(AbstractTest):
    ...
    def run(
        ...
        **kwargs,
        ...
        Accepts a Generator (any subclass, including `CombinedF2LinearSource`
        or `DigitalNet`) or — during the Phase-1 migration window — a
        Python `Combination`. The inner loop runs in C++.
        """
        if not self.cfverif:
        ...
        kg = cpp_gen.k()
        L = cpp_gen.L()
        L_for_phi4 = me_results.L if me_results is not None else L

        result = _cpp.run_collision_free(cpp_gen, kg, L, L_for_phi4)
        return CollisionFreeResults(
    ...
    @staticmethod
    def _compute_phi4(kg: int, L: int) -> list[bool]:
        """SetPhi4: dimensions whose CF rank must be checked. Delegates to C++."""
        import regpoly._regpoly_cpp as _cpp
        return list(_cpp.compute_phi4(kg, L))

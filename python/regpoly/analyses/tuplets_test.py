...
class TupletsTest(AbstractTest):
    ...
    def run(self, gen_or_C, *args, **kwargs) -> TupletsResults:
        """
        TestTuplets: compute Delta(t_1,...,t_d). Accepts a Generator
        (any subclass) or — during the Phase-1 migration window — a
        Python Combination. Orchestration loop lives in C++ as
        `run_tuplets`.
        """
        ...
        result = _cpp.run_tuplets(
            cpp_gen, kg, L, self.d, list(self.s),

...
using regpoly::core::F2LinearSource;
...
public:
    ...
    std::unique_ptr<ITestable> copy() const override {
        ...
        return std::make_unique<IdentityNet>(*this);
    }
...
TEST(DigitalNet, PrepareBuildsRepeatedIdentityRows) {
    const int m = 4;
    IdentityNet net(m, m);
    std::vector<const F2LinearSource*> gens = {&net};
    std::vector<int> gen_k = {m};
    GaussMatrix mat = GaussMatrix::prepare(gens, gen_k,
                                           /*kg=*/m,
                                           /*indice_max=*/m,
                                           /*L=*/m);

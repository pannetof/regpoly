...
// a pre-v2 carry32_624_31_final.dat fixture through the .dat reader,
...
// same parameters the pre-v2 `carry32_624_31_final.dat` fixture would
// have produced via the (now-removed) `.dat` path.
std::unique_ptr<Recurrence> make_well19937a(int L) {
    ParamBag p;
    p.set_int("w", 32);
    p.set_int("r", 624);
    p.set_int("p", 31);

...
//
// This driver iterates a ComboEnumerator, fires a Python-supplied on_try
...
TEST(TemperingSearchDriver, IteratesEveryComboNbTriesTimes) {
    ...
    auto on_combo_start = [&](ComboEnumerator&, int) { ++combo_starts; };
    auto on_try = [&](ComboEnumerator&, int, int, bool) {
        ++try_calls;
        TemperingTryResult out;
        out.got_result = true;
        out.score = 1;        // constant score; never breaks early
        return out;
    };
    auto on_combo_done = [&](ComboEnumerator&, int, int, int) { ++combo_dones; };
...
TEST(TemperingSearchDriver, EarlyStopOnZeroScore) {
    ...
    c.pool(0).add_source(*make_tgfsr(7));
    ASSERT_TRUE(c.reset());
    ...
    auto on_try = [&](ComboEnumerator&, int, int t, bool) {
        ++try_calls;
        TemperingTryResult out;
        out.got_result = true;
    ...
    auto on_combo_done =
        [&](ComboEnumerator&, int, int best_score, int best_try) {
...
TEST(TemperingSearchDriver, TracksMinimumScorePerCombo) {
    ...
    auto on_combo_start = [&](ComboEnumerator&, int) { ++combo_seen; };
    auto on_try = [&](ComboEnumerator&, int, int t, bool) {
        TemperingTryResult out;
        out.got_result = true;
...
TEST(TemperingSearchDriver, FailedTriesNotCounted) {
    ...
    auto on_combo_done =
        [&](ComboEnumerator&, int, int, int best_try) {
...
TEST(TemperingSearchDriver, IsFirstFlagOnFirstTryOfEachCombo) {
    ...
    auto on_try = [&](ComboEnumerator&, int, int, bool is_first) {
    ...
    auto on_combo_done = [&](ComboEnumerator&, int, int, int) {};
...
TEST(TemperingSearchDriver, ProgressFiresOnInterval) {
    ...
    for (uint32_t a : {1u, 2u, 3u, 4u, 5u}) {
        c.pool(0).add_source(*make_tgfsr(a));
    }
    ASSERT_TRUE(c.reset());

...
void register_all_transformations() {
    static const int once = []{
        TR::reg("permut",
                [](const ParamBag& p){ return PermutationTrans::from_params(p); },
                &PermutationTrans::param_specs);
        TR::reg("tempMK",
                [](const ParamBag& p){ return TemperMKTrans::from_params("tempMK", p); },
                &TemperMKTrans::param_specs);
        TR::reg("tempMK2",
                [](const ParamBag& p){ return TemperMKTrans::from_params("tempMK2", p); },
                &TemperMKTrans::param_specs);
        TR::reg("laggedTempering",
                [](const ParamBag& p){ return LaggedTemperingTrans::from_params(p); },
                &LaggedTemperingTrans::param_specs);
        return 0;
    }();
    (void)once;
}
...
std::unique_ptr<Recurrence> create_generator(
    const std::string& family, const ParamBag& params, int L)
{
...
std::unique_ptr<ParameterSpaceEnumerator> make_parameter_space_enumerator(
    const std::string& family, const ParamBag& resolved, int L)
{
    register_all_generators();
    auto* info = GR::find(family);
...
std::unique_ptr<Transformation> create_transformation(
    const std::string& type, const ParamBag& params)
{

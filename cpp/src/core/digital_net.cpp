...
DigitalNet::DigitalNet(int m, int s_max)
    : F2LinearSource(m, m), state_(m), j_(0), s_max_(s_max) {
    if (m <= 0) {
...
BitVect DigitalNet::raw_output() const {
    if (j_ < 1 || j_ > s_max_) {
        std::ostringstream oss;
        oss << "DigitalNet::raw_output: coordinate j_ = " << j_
...
DigitalNet::default_test_method(const std::string& test_type) const {

...
BitVect Recurrence::raw_output() const {
    BitVect out(L_);
...
void Recurrence::get_transition_state(uint64_t* out_words, int out_nwords) const {
...
BitVect Recurrence::char_poly() const {
    int K = k_;
    BitVect init_bv(K);
    init_bv.set_bit(0, 1);
    BitVect result;
...
bool Recurrence::is_full_period() const {
    int K = k_;
...
std::optional<std::string> Recurrence::default_test_method(const std::string& test_type) const {
...
std::optional<std::string> Recurrence::compute_default_test_method(const std::string& test_type) const {
...
std::vector<Recurrence*> Recurrence::components() const {
    return {const_cast<Recurrence*>(this)};
}


// ── Transition matrix ────────────────────────────────────────────────────

std::vector<BitVect> Recurrence::transition_matrix() const {
    int K = k_;
    auto gen = clone_recurrence();

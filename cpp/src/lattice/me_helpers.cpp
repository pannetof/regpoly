...
static NTL::GF2X char_poly_to_gf2x(Recurrence* gen) {
...
BitVect polychar_comb(const CombinedF2LinearSource& cs) {
    auto gens = cs.recurrence_components("polychar_comb");
    NTL::GF2X product;
...
{
    auto gens = cs.recurrence_components("find_polys");
    int J = (int)gens.size();
...
{
    // Validate components upfront so the throw fires at kernel entry,
    // not deep inside polychar_comb / find_polys.
    (void)cs.recurrence_components("test_me_lat");
    ...
    // certified by construction. The unified result type now carries this.
    result.verified = true;

    // 1. Compute combined characteristic polynomial
    BitVect M = polychar_comb(cs);

    // 2-3. Find and normalize generating polynomials
    std::vector<BitVect> polys;
    find_polys(cs, Deg, M, polys, RES);

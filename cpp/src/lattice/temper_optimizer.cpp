...
// ══════════════════════════════════════════════════════════════════════════
// TemperingOptimizerCache implementation
// ══════════════════════════════════════════════════════════════════════════

TemperingOptimizerCache::TemperingOptimizerCache(
    const CombinedF2LinearSource& cs,
    int kg, int L)
    : cs_(&cs),
      gens_(cs.recurrence_components("TemperingOptimizerCache")),
      kg_orig_(kg), Deg_(kg), L_(L), RES_(std::min(L, kg))
{
    ...
    find_polys(cs, Deg_, M_, polys_, RES_);
    ...
    find_polys(cs, Deg_, M_, raw_polys, 1);
...
std::vector<int> TemperingOptimizerCache::compute_all() {
...
std::vector<int> TemperingOptimizerCache::rebuild() {
    ...
    find_polys(*cs_, Deg_, M_, polys_, RES_);
...
void TemperingOptimizerCache::reset_step() {
    previous_v_ = 0;
    current_base_.reset();
    opt_stack_.clear();
...
int TemperingOptimizerCache::step(int v) {
...
void TemperingOptimizerCache::find_single_poly(int idx, BitVect& out_poly) {
    ...
    for (int j = 0; j < J; j++)
        gen_copies.push_back(gens_[j]->clone_recurrence());
    ...
    for (int k = 0; k < K; k++) {
        ...
        combined.zero();
        for (int j = 0; j < J; j++) {
            combined.xor_with(gen_copies[j]->get_output());
        }
...
void TemperingOptimizerCache::normalize_single(BitVect& poly) {
...
void TemperingOptimizerCache::refresh_inv_g0() {
    // Recompute raw g_0 for the current bitmask
    std::vector<BitVect> raw_polys;
    find_polys(*cs_, Deg_, M_, raw_polys, 1);
...
int TemperingOptimizerCache::compute_gap(int v) {
    if (v < 1 || v > RES_) return 0;

...
std::string RMT64Gen::name() const { return "RMT64Gen"; }

std::string RMT64Gen::display_str() const {
    char buf[160];
...
void RMT64Gen::init(const BitVect& init_bv) {
    state_ = BitVect(k_);
...
std::unique_ptr<ITestable> RMT64Gen::copy() const {
    auto p = std::make_unique<RMT64Gen>(mexp_, pos_, mata_, maskb_, maskc_, L_);
...
BitVect RMT64Gen::raw_output() const {
    BitVect out(L_);
    int n = std::min(L_, 64);
    for (int i = 0; i < n; i++) {
...
std::unique_ptr<Recurrence> RMT64Gen::from_params(const ParamBag& params, int L) {
    ...
    return std::make_unique<RMT64Gen>(mexp, pos, mata, maskb, maskc,
                                   std::min(L, 64));
}

std::vector<ParamSpec> RMT64Gen::param_specs() {
    return {

...
PolyLCGGen::PolyLCGGen(int k, const BitVect& poly, int L)
    : Recurrence(k, L), poly_(poly.copy()) {}
...
std::unique_ptr<ITestable> PolyLCGGen::copy() const {
...
std::unique_ptr<Recurrence> PolyLCGGen::from_params(const ParamBag& params, int L) {

...
TGFSRGen::TGFSRGen(int w, int r, int m, const BitVect& a, int L)
    : Recurrence(w * r, L), w_(w), r_(r), m_(m), a_(a.copy()) {}
...
std::unique_ptr<ITestable> TGFSRGen::copy() const {
...
std::unique_ptr<Recurrence> TGFSRGen::from_params(const ParamBag& params, int L) {

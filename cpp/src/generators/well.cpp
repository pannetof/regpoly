...
BitVect WELLGen::raw_output() const {
    BitVect out(L_);
    int nw = (L_ + 31) / 32;
    for (int j = 0; j < nw; j++) {
...
std::unique_ptr<ITestable> WELLGen::copy() const {
...
void reject_legacy_triple(const ParamBag& params) {
...
std::unique_ptr<Recurrence> WELLGen::from_params(const ParamBag& params, int L) {

...
std::string MTGen::name() const { return "Mersenne Twister"; }

std::string MTGen::display_str() const {
    char buf[128];
...
void MTGen::init(const BitVect& init_bv) {
    state_ = BitVect(state_bits_);
...
BitVect MTGen::rotated_state() const {
...
BitVect MTGen::raw_output() const {
...
void MTGen::get_transition_state(uint64_t* out_words, int out_nwords) const {
    BitVect rot = rotated_state();
    BitVect tmp(k_);
...
std::unique_ptr<ITestable> MTGen::copy() const {
    auto p = std::make_unique<MTGen>(w_, r_, m_, p_, a_, L_);
...
uint64_t MTGen::V(int idx) const {
    int ii = r_ - idx - 1;
    return state_.get_word(ii, w_);
}

void MTGen::SetV(int idx, uint64_t val) {
    int ii = r_ - idx - 1;
...
std::unique_ptr<Recurrence> MTGen::from_params(const ParamBag& params, int L) {
    ...
    return std::make_unique<MTGen>(w, r, m, p, a, L);
}

std::vector<ParamSpec> MTGen::param_specs() {
    return {

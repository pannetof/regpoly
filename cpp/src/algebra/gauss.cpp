...
// Phase 2 Step 6b: chain application moved INTO each component via
// `F2LinearSource::tempered_output()`. The prepare helper just copies
// the tempered bits into the row buffer at the right offset.
static void copy_L_bits_into_row(
    const BitVect& tempered,
    ...
    uint64_t* row_words, int row_nwords)
{
    for (int b = 0; b < L; b++) {
        int src_word = b / 64;
        int src_bit = 63 - (b % 64);
        if (src_word < tempered.nwords()
            && ((tempered.data()[src_word] >> src_bit) & 1)) {
...
GaussMatrix GaussMatrix::prepare(
    const std::vector<const F2LinearSource*>& gens,
    ...
    int kg, int indice_max, int L)
{
    ...
    for (int j = 0; j < (int)gens.size(); j++) {
        const F2LinearSource* gen = gens[j];
        int k = gen_k[j];
        ...
        for (int gl = 0; gl < k; gl++) {
            auto gen_copy = gen->clone_source();
            gen_copy->init(bc);
            bc.rshift(1);

            std::fill(row_words.begin(), row_words.end(), 0ULL);

            copy_L_bits_into_row(gen_copy->get_output(),
                                 L, 0, row_words.data(), row_nwords);

            for (int mc = 1; mc < indice_max; mc++) {
                gen_copy->next();
                copy_L_bits_into_row(gen_copy->get_output(),
                                     L, mc * L, row_words.data(), row_nwords);
            }

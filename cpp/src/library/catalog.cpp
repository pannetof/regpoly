...
// Push a ParamValue into a runtime ParamBag object so the dry-build
// step can call create_generator / create_transformation. Mirrors
// the Python factory's lenient int/string coercion.
void apply_param(ParamBag& dst, const std::string& key, const ParamValue& v) {
    switch (v.kind) {
        case ParamKind::Int:
...
std::vector<CatalogComponent> parse_components(const YAML::Node& n,
                                        std::vector<std::string>& errors) {
...
void validate_pdf_exists(Paper& paper) {
    ...
    // `paper.pdf` is relative to the catalog data root — the parent of the
    // library dir (e.g. "papers/foo.pdf" -> <_data>/papers/foo.pdf, with the
    // catalog YAML at <_data>/library/foo.yaml). This keeps papers a sibling
    // of the catalog so resolution is location-independent (source tree,
    // installed share/, or wheel package data).
    std::filesystem::path src(paper.source_path);
    std::filesystem::path data_root = src.parent_path().parent_path();
    std::filesystem::path pdf = data_root / paper.pdf;
    std::error_code ec;
...
void dry_build_generator(const Paper& paper, CatalogGenerator& g) {
    ...
    for (auto& comp : g.components) {
        ...
        // steps that omit it (mirrors Python: Recurrence.params.get("w")).

 ...
 * `ComboEnumerator`, for each combination + retry runs the configured
 ...
 *
 * Internally this loop builds polymorphic `SearchPredicate` objects (see
...
struct SeekTestSpec {
     ...
     *
     * Looked up via `EquidistributionMethodRegistry`. When `kind` is one of the legacy
...
using SeekOnPrepFn =
    std::function<void(ComboEnumerator&, bool /*is_retry*/)>;
/// Per-iteration callback. Fires once after every iteration's test stack completes.
using SeekOnIterFn =
    std::function<void(ComboEnumerator&, const SeekIterResult&)>;
/// Periodic progress callback.
using SeekOnProgressFn =
    std::function<void(const SeekProgress&)>;

/**
 * @brief Run the seek-search loop over a configured `ComboEnumerator`.
 *
 * Each iteration:
 ...
 *                   100, nullptr,
 *                   [](ComboEnumerator&, const SeekIterResult& r) {
 *                       // Persist r.
 *                   },
 *                   nullptr);
 * @endcode

 ...
 *
 *   - a `TemperingOptimizerCache` (already-existing C++ stateful kernel that
 ...
 * @ingroup core
 */
struct TemperingOptimizerResult {
 ...
 * @param cache       Stateful tempering cache (one per `ComboEnumerator`).
 ...
 * @see :py:class:`regpoly.search.tempering_optimizer.TemperingOptimizer`
 */
TemperingOptimizerResult run_tempering_optimizer_once(
 ...
 * @see :py:class:`regpoly.search.tempering_optimizer.TemperingOptimizer`
 */
TemperingOptimizerResult run_tempering_optimizer_minimize(
    const TemperingOptimizerConfig& cfg,
    TemperingOptimizerCache& cache,
    std::vector<TemperParamLocator>& params,
    const std::vector<std::vector<uint64_t>>& safe_masks);

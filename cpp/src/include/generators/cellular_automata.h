 ...
 * lazily by the base `Recurrence::transition_matrix()` helper when
 ...
 * a `CombinedF2LinearSource`, the paper's convention treats the combined
 ...
 * @ingroup core
 */
class CellularAutomataGen : public Recurrence {
public:
    /**
    ...
    /**
     * @brief Build a CellularAutomataGen from a ParamBag dict (registry factory hook).
     * @param params  Parameter dict with keys k, rule150_positions, s.
     * @param L       Output resolution in bits.
     * @return        A constructed CellularAutomataGen as a polymorphic Recurrence pointer.
...
protected:
     ...
     * right default — even when wrapped in a CombinedF2LinearSource, the

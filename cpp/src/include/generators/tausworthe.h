 ...
 * @ingroup core
 */
class TauswortheGen : public Recurrence {
public:
    /**
    ...
    /**
     * @brief Build a TauswortheGen from a ParamBag dict (registry factory hook).
     * @param params  Parameter dict with keys k, nb_terms, quicktaus, poly, s.
     * @param L       Output resolution in bits.
     * @return        A constructed TauswortheGen as a polymorphic Recurrence pointer.
     * @throws std::runtime_error  If a required parameter is missing or invalid.
     */
    static std::unique_ptr<Recurrence> from_params(const ParamBag& params, int L);
     ...
     *
     * The ParamBag bag carries the structural inputs (`k`, `nb_terms`,
     * `quicktaus`) plus an optional `s` that, if present, fixes the
     * decimation step.
     *
     * @param resolved  ParamBag with k, nb_terms, quicktaus pinned (s optional).
     * @param L         Output resolution in bits.
     ...
     */
    static std::unique_ptr<ParameterSpaceEnumerator> make_enumerator(

 ...
 * @ingroup core
 */
class SFMTGen : public Recurrence {
public:
    /**
    ...
    /**
     * @brief Build an SFMTGen from a ParamBag dict (registry factory hook).
     * @param params  Parameter dict with keys mexp, pos1, sl1, sl2, sr1, sr2,
     *                msk1..msk4.
     * @param L       Output resolution in bits.
     * @return        A constructed SFMTGen as a polymorphic Recurrence pointer.
     * @throws std::runtime_error  If a required parameter is missing or invalid.
     */
    static std::unique_ptr<Recurrence> from_params(
     ...
     * this via the abstract `Recurrence` API and partitions the
    ...
    void simd_add_state(const Recurrence& other) override;

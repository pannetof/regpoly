 ...
 * @ingroup core
 *
 * Defines the polynomial-vector primitive `DualLatticeBasisRow` and the
 * `DualLatticeBase` that drives the dual-lattice equidistribution
 * method. Selected via `EquidistributionMethodRegistry` by the name string
 * `"lattice"` in the YAML config.
 *
 * @verbatim
 *   DualLatticeBasisRow: a basis vector = an l-tuple of polynomials over GF(2).
 *
 ...
 *
 * Each `DualLatticeBasisRow` is an `l`-tuple of polynomials over `GF(2)`, stored
...
struct DualLatticeBasisRow {
    ...
    DualLatticeBasisRow() = default;
    /**
     * @brief Construct a `DualLatticeBasisRow` with `resolution_` active coordinates
     *        and per-coordinate capacity for degree up to `degmax`.
     *
     * Storage is allocated for `resolution_` coordinates; if you need
     * to grow beyond that, construct a wider `DualLatticeBasisRow`. `DualLatticeBase`
     * pre-sizes its `DualLatticeBasisRow`s at `maxresolution` and then sets each
     ...
     */
    DualLatticeBasisRow(int resolution_, int degmax);
    ...
    void swap(DualLatticeBasisRow& other);
    /**
     ...
     */
    void assign_shifted(const DualLatticeBasisRow& a, int s);
     ...
     */
    void add_shifted(const DualLatticeBasisRow& a, int s);
     ...
     * @param s  Shift amount.
     */
    void add_self_mult(const DualLatticeBasisRow& a, int s);
     ...
     */
    void assign_xor(const DualLatticeBasisRow& a, DualLatticeBasisRow& b);
};
 ...
 *    `degmax`).
 *
 * Per-`DualLatticeBasisRow` operations (degree update, shift, XOR, etc.) live on
 * `DualLatticeBasisRow` itself. The only method here that touches a single
 * `DualLatticeBasisRow` is `element_norme_equal`, which indexes through `perm_`
 * and is therefore basis-state-bound.
 *
 * Selected via `EquidistributionMethodRegistry` by the name string `"lattice"`.
 *
 * @ingroup core
 */
...
private:
    std::vector<DualLatticeBasisRow> vect_;
    ...
    DualLatticeBasisRow& vb(int i) { return vect_[i]; }
    const DualLatticeBasisRow& vb(int i) const { return vect_[i]; }

    // Set the active resolution on every basis DualLatticeBasisRow. Called whenever
    // resolution_ changes (dual_base / dual_base_increase) so each
    // DualLatticeBasisRow knows how many of its coordinates are live.
    ...
    // rather than on DualLatticeBasisRow.
    int element_norme_equal(const DualLatticeBasisRow& P, int j, int norme) const;

 ...
 * implied by a `Recurrence` (typically a `DigitalNet`, but any
 * `Recurrence` works — the kernel sees only the F_2-linear matrix that
 ...
 * pattern as `test_me_matricial`: bail when any
 * `tvals[s] > delta[s]` or running `se > max_t_sum`.
 *
 * @param gen        Recurrence (typically a `DigitalNet` or a
 *                   `CombinedF2LinearSource` wrapping one).

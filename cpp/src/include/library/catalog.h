 ...
 * | `CatalogGenerator`  | `regpoly.library.Recurrence`    |
 ...
 *
 * Combined generators expose several `CatalogComponent` entries; single-family
 ...
 * generators), and a list of `CatalogComponent` definitions that the
 * factory uses to materialise a runtime `regpoly::core::Recurrence`.
...
struct CatalogGenerator {
    ...
    std::vector<CatalogComponent> components;   ///< CatalogComponent definitions (1+ entries).
...
public:
     ...
     * @param gen_id  Recurrence id (unique across the catalog).
 ...
 * @param params     CatalogComponent parameter map.

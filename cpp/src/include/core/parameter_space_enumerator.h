...
/**
 * @file parameter_space_enumerator.h
 * @brief Abstract interface for the exhaustive-search generator-space enumerator.
 *
 * Defines `ParameterSpaceEnumerator` (the abstract interface the worker and
 * API talk to), the registry helpers `make_parameter_space_enumerator` /
 ...
 *   auto en = make_parameter_space_enumerator("TauswortheGen", resolved, 32);
 *   if (en) {
 *       auto total = en->size_dec();        // decimal string
 *       auto axes  = en->axes();
 *       ParamBag p   = en->at("0");           // first combination
 *   }
 * @endcode
 *
 * @ingroup core
...
public:
    ...
    virtual ~ParameterSpaceEnumerator() = default;
     ...
     * across processes — same inputs produce the same `ParamBag`.
     *
     * @param idx_dec  Index as a decimal string (may exceed `2^63`).
     * @return         Concrete `ParamBag` for that combination.
     */
    virtual ParamBag at(const std::string& idx_dec) const = 0;
...
/**
 * @brief Construct a `ParameterSpaceEnumerator` for a given family if one is registered.
 *
 ...
 * @param resolved  Resolved `ParamBag` after YAML / spec processing.

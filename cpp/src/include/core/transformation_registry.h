...
public:
    /** @brief Factory callable: build a `Transformation` from a `ParamBag` bag. */
    using FromParamsFn = std::function<
        std::unique_ptr<Transformation>(const ParamBag& params)>;
     ...
     * @param from_params     Factory callable taking a `ParamBag` bag.

 ...
 * @brief Self-registering registry of every concrete `Recurrence` subclass.
 *
 ...
 * `py::class_<FooGen, Recurrence>(m, "FooGen")` (only needed when the
 ...
 * `family_is_enumerable`, `make_parameter_space_enumerator`) look up the
...
public:
    /** @brief Factory callable: build a `Recurrence` from a `ParamBag` bag and width `L`. */
    using FromParamsFn = std::function<
        std::unique_ptr<Recurrence>(const ParamBag& params, int L)>;
    ...
    using EnumeratorFn = std::function<
        std::unique_ptr<ParameterSpaceEnumerator>(const ParamBag& resolved, int L)>;
...
/**
 * @brief Self-register a `Recurrence` subclass at translation-unit init time.
 *
...
/**
 * @brief Self-register a `Recurrence` subclass together with an enumerator factory.
 *
 * Same as `REGISTER_GENERATOR` plus a fourth argument that supplies
 * the exhaustive `ParameterSpaceEnumerator` factory. `TauswortheGen` is the
 * canonical example.
 *
 * @param Cls         Subclass type name.

 ...
 * `components` / `tests`), build the corresponding `ComboEnumerator` +
 ...
 * `Recurrence.from_yaml`) is intentionally NOT supported in this
...
using regpoly::core::Recurrence;
using regpoly::core::ParamBag;
...
struct ComponentSpec {
    ...
    ParamBag common_params;               ///< Parameters shared by every generator entry.
    std::vector<ParamBag> per_gen_params; ///< Per-generator parameter overrides.
 ...
 *   regpoly::core::ComboEnumerator& comb = *built.combination;
 *   auto combined = regpoly::core::build_combined_from_enumerator(comb);
 ...
 *
 * Carries the constructed `ComboEnumerator` plus the owning vectors of
 * generators / transformations. `ComboEnumerator::add_gen` /
 ...
 */
struct SeekBuild {
    std::unique_ptr<ComboEnumerator> combination;                                       ///< Constructed iterator.
    std::vector<std::vector<std::unique_ptr<Recurrence>>> per_component_gens;        ///< Owning generator pools.
    std::vector<std::vector<std::unique_ptr<Transformation>>> per_component_trans;  ///< Owning tempering chains.
};

/**
 * @brief Build a fully-initialised `ComboEnumerator` from a `SeekConfig`.
 *
 * Resolves `Inline` / `Same` into actual `Recurrence` objects via
 * the existing factories, attaches the tempering chain to each
 * component, and calls `reset()`.
 *
 * @param cfg  Parsed seek configuration.
 * @return     The constructed `ComboEnumerator` plus the owning
 *             per-component generator / tempering vectors.
 */
SeekBuild build_search(const SeekConfig& cfg);
